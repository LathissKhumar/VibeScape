...
- Problem: Dynamic imports (next/dynamic with ssr:false) don't render synchronously in jsdom test environment. This affects ListeningHeatmapSection and AudioRadarSection tests. 
  - Mitigation: mock the dynamically-imported component with `vi.mock` and wrap assertions in `waitFor`. 
  - For AudioRadarSection: changed `getByTestId("audio-radar")` to `await waitFor(() => screen.getByTestId("audio-radar"))`.
  - A better approach would be to make the dynamic import a prop or HOC that tests can override.
